...
class DownBlock(torch.nn.Module):
    ...
    def forward(self, out_vae_noise, out_encoder, time):
        outs = []

        out_vae_noise = self.res0(out_vae_noise, time)
        out_vae_noise = self.tf0(out_vae_noise, out_encoder)
        outs.append(out_vae_noise)

        out_vae_noise = self.res1(out_vae_noise, time)
        out_vae_noise = self.tf1(out_vae_noise, out_encoder)
        outs.append(out_vae_noise)

        out_vae_noise = self.out(out_vae_noise)
        outs.append(out_vae_noise)

        return out_vae_noise, outs
...
class UpBlock(torch.nn.Module):
    ...
    def forward(self, out_vae_noise, out_encoder, time, out_down):
        out_vae_noise = self.res0(torch.cat([out_vae_noise, out_down.pop()], dim=1), time)
        out_vae_noise = self.tf0(out_vae_noise, out_encoder)

        out_vae_noise = self.res1(torch.cat([out_vae_noise, out_down.pop()], dim=1), time)
        out_vae_noise = self.tf1(out_vae_noise, out_encoder)

        out_vae_noise = self.res2(torch.cat([out_vae_noise, out_down.pop()], dim=1), time)
        out_vae_noise = self.tf2(out_vae_noise, out_encoder)

        if self.out:
            out_vae_noise = self.out(out_vae_noise)

        return out_vae_noise
...
class UNet(torch.nn.Module):
    ...
    def forward(self, out_vae_noise, out_encoder, time, down_block_additional_residuals=None, mid_block_additional_residual=None):
        """
        ...
        # [1, 1280, 8, 8]
        # [1, 1280, 8, 8]
        out_down = [out_vae_noise]

        # [1, 320, 64, 64],[1, 77, 768],[1, 1280] -> [1, 320, 32, 32]
        # out -> [1, 320, 64, 64],[1, 320, 64, 64][1, 320, 32, 32]
        out_vae_noise, out = self.down_block0(out_vae_noise=out_vae_noise, out_encoder=out_encoder, time=time)
        out_down.extend(out)

        # [1, 320, 32, 32],[1, 77, 768],[1, 1280] -> [1, 640, 16, 16]
        # out -> [1, 640, 32, 32],[1, 640, 32, 32],[1, 640, 16, 16]
        out_vae_noise, out = self.down_block1(out_vae_noise=out_vae_noise, out_encoder=out_encoder, time=time)
        out_down.extend(out)

        # [1, 640, 16, 16],[1, 77, 768],[1, 1280] -> [1, 1280, 8, 8]
        # out -> [1, 1280, 16, 16],[1, 1280, 16, 16],[1, 1280, 8, 8]
        out_vae_noise, out = self.down_block2(out_vae_noise=out_vae_noise, out_encoder=out_encoder, time=time)
        out_down.extend(out)

        # [1, 1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.down_res0(out_vae_noise, time)
        out_down.append(out_vae_noise)

        # [1, 1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.down_res1(out_vae_noise, time)
        out_down.append(out_vae_noise)

        # control down
        if down_block_additional_residuals is not None:
            for idx in range(12):
                out_down[idx] += down_block_additional_residuals[idx]

        # ----mid----
        # [1, 1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.mid_res0(out_vae_noise, time)

        # [1, 1280, 8, 8],[1, 77, 768] -> [1, 1280, 8, 8]
        out_vae_noise = self.mid_tf(out_vae_noise, out_encoder)

        # [1, 1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.mid_res1(out_vae_noise, time)

        # control mid
        if mid_block_additional_residual is not None:
            out_vae_noise += mid_block_additional_residual


        # ----up----
        # [1, 1280+1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.up_res0(torch.cat([out_vae_noise, out_down.pop()], dim=1), time)

        # [1, 1280+1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.up_res1(torch.cat([out_vae_noise, out_down.pop()], dim=1), time)

        # [1, 1280+1280, 8, 8],[1, 1280] -> [1, 1280, 8, 8]
        out_vae_noise = self.up_res2(torch.cat([out_vae_noise, out_down.pop()], dim=1), time)

        # [1, 1280, 8, 8] -> [1, 1280, 16, 16]
        out_vae_noise = self.up_in(out_vae_noise)

        # [1, 1280, 16, 16],[1, 77, 768],[1, 1280] -> [1, 1280, 32, 32]
        # out_down -> [1, 640, 16, 16],[1, 1280, 16, 16],[1, 1280, 16, 16]
        out_vae_noise = self.up_block0(out_vae_noise=out_vae_noise, out_encoder=out_encoder, time=time, out_down=out_down)

        # [1, 1280, 32, 32],[1, 77, 768],[1, 1280] -> [1, 640, 64, 64]
        # out_down -> [1, 320, 32, 32],[1, 640, 32, 32],[1, 640, 32, 32]
        out_vae_noise = self.up_block1(out_vae_noise=out_vae_noise, out_encoder=out_encoder, time=time, out_down=out_down)

        # [1, 640, 64, 64],[1, 77, 768],[1, 1280] -> [1, 320, 64, 64]
        # out_down -> [1, 320, 64, 64],[1, 320, 64, 64],[1, 320, 64, 64]
        out_vae_noise = self.up_block2(out_vae_noise=out_vae_noise, out_encoder=out_encoder, time=time, out_down=out_down)

        # ----out----
        # [1, 320, 64, 64] -> [1, 4, 64, 64]
        out_vae_denoised = self.out(out_vae_noise)

        return out_vae_denoised
...
if __name__ == '__main__':
    ...
    o = u(out_vae_noise, out_con, timescale)
    print(o.shape)