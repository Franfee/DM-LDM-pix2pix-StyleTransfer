...
class ImageDataset(Dataset):
    def __init__(self, root, transforms_=None, unaligned=False, mode="train"):
        ...
        self.files_A = sorted(glob.glob(os.path.join(root, "%s2017" % mode) + "/*.*"))
    ...
    def __getitem__(self, index):
        ...
        item_A = self.vae.sample(self.vae.encoder(item_A)).squeeze(0).cpu()

        #item_B = self.transform(image_B)
        # return {"lantent": item_A, "content": canny_map,"style": item_B}
        return {"lantent": item_A, "content": canny_map}
